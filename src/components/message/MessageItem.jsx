  ...
  return (
    ...
    >
      {msg?.content.length > 0 && (
        <div
          className={`relative py-[8px] px-[16px] max-w-[70%] rounded-lg break-words word-wrap ${
            msg?.sender._id == currentUser._id
              ? "bg-[#e3e3e5] dark:bg-gray-700"
              : "bg-[#f1f1f2] dark:bg-gray-800"
          }`}
        >
          <span className='text-[#4A4B4D] dark:text-gray-100 text-[18px] break-words'>{msg?.content}</span>

          {msg?.sender._id == currentUser._id && (
            <div className='absolute top-[50%] translate-y-[-50%] left-[-35px] py-[4px] px-[6px] w-[30px] h-[30px] rounded-full cursor-pointer hover:bg-[#cccfd4] dark:hover:bg-gray-700 group'>
      ...
      {msg?.image && (
        <>
          ...
          {msg?.sender._id == currentUser._id && (
            <div className='absolute top-[50%] translate-y-[-50%] left-[40%] py-[4px] px-[6px] w-[30px] h-[30px] rounded-full cursor-pointer hover:bg-[#cccfd4] dark:hover:bg-gray-700 group'>
              <FaEllipsis className='text-[18px] text-subTextColor dark:text-subTextColor-dark translate-y-[2px]' />
