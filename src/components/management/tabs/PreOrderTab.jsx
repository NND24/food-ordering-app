...
const OrderCard = ({ order }) => {
  return (
    <Link href={`orders/${order.id}/preorder`} passHref>
      <div className="border rounded-lg shadow-md p-4 bg-white mb-4">
        {/* Order Header */}
        <div className="flex justify-between items-center mb-2">
          <div className="flex items-center">
            <div className="text-sm text-gray-700">
              <Link href={`orders/${order.id}/preorder`} className="text-gray-700 text-md font-bold">
                {order.user}
              </Link>
      ...
      {orderList.map((dateGroup, index) => (
        <div key={index}>
          <div className="mb-2 shadow-md bg-white p-2 rounded-sm grid grid-cols-3">
            <p className="text-sm font-medium text-gray-600 col-span-2">{renderDateHeader(dateGroup.date)}</p>
          </div>
